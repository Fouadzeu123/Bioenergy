        ...
        <form method="POST" action="{{ route('admin.users.update', $user->id) }}" class="p-8 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="username" value="{{ old('username', $user->username) }}" required
                           class="w-full border-2 border-gray-100 rounded-xl px-5 py-3 focus:outline-none focus:border-emerald-500 transition">
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Niveau VIP</label>
                    <select name="level" class="w-full border-2 border-gray-100 rounded-xl px-5 py-3 focus:outline-none focus:border-emerald-500 transition">
                        @for($i = 0; $i <= 10; $i++)
                ...
                <div>
                    ...
                    <select name="role" class="w-full border-2 border-gray-100 rounded-xl px-5 py-3 focus:outline-none focus:border-emerald-500 transition">
                        <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>Utilisateur</option>
                        <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Administrateur</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Solde actuel ({{ $user->currency }})</label>
            ...
            <div class="flex justify-end gap-4 pt-6 border-t">
                <a href="{{ route('admin.users.show', $user->id) }}"
                   class="px-8 py-4 bg-gray-100 rounded-xl font-bold hover:bg-gray-200 transition text-gray-600">
                    Annuler
                </a>
                <button type="submit"
                        class="px-8 py-4 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-100">
                    Sauvegarder
                </button>
            </div>
        </form>
